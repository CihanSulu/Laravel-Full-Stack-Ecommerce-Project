(function(window, document, $, undefined) {
    ...
    var axilInit = {
        ...
        axilSlickActivation: function(e) {
            ...
            $('.new-arrivals-product-activation-3').slick({
                infinite: true,
                slidesToShow: 6,
                slidesToScroll: 4,
                arrows: true,
...
$(".starclick").click(function(){
    ...
    $(".reating-inner a").each(function(index, element) {
        $(this).find("i").removeClass("fas")
        $(this).find("i").removeClass("text-warning")
        $(this).find("i").addClass("far")

        if(index+1 <= parseInt(star)){
...
$(".comment-inner").each(function(index, element) {
    let date = $(this).find("small").html()
    let saat = parseInt(date.substr(16,2))
    let dakika = parseInt(date.substr(19,2))
    if(saat>27)
        saat = saat % 27
    if(dakika>27)
        dakika = dakika % 27
    
    let upper = ['A', 'B', 'C', 'Ç', 'D', 'E', 'F', 'G','H', 'İ', 'J', 'K', 'L', 'M', 'N', 'O', 'Ö', 'P', 'R', 'S', 'Ş', 'T', 'U', 'Ü', 'V', 'Y', 'Z']
    $(this).find("span").find("span").find("span").html(upper[saat] + "*** " + upper[dakika] + "***")
});
