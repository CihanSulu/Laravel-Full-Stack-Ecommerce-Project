...
@php
    if(!isset($category)){
        ...
        if(!is_null(request('e')))
            ...
            $category->kategori_adi = "Ürünlerimiz";
    }

@endphp
@section('title','En İyi '.$category->kategori_adi.' | '.$site)
@section('description','En Uygun Fiyatlı '.$category->kategori_adi.', '.$category->kategori_adi.' Satın Al')
@section('keywords',$category->kategori_adi.' Satın Al, '.$category->kategori_adi.' Tester Satın Al, En Ucuz '.$category->kategori_adi)
                            ...
                            <ul class="pagination justify-content-center">
                                ...
                                @if($total_products > 24)
                                    ...
                                    @if($page > 1)
                                        ...
                                        <li><a class="prev" href="{{ request()->fullUrlWithQuery(['page' => $onceki]) }}">Önceki</a></li>
                                    @endif
                                    @if($page == 1)
                                        <li><a href="javascript:void(0);" class="active">1</a></li>
                                    @else
                                        <li><a href="{{ request()->fullUrlWithQuery(['page' => '1']) }}">1</a></li>
                                    @endif
                                    ...
                                    @if($page+$x < $sonSayfa-1)
                                        <li><a href="javascript:void(0);">...</a></li>
                                        <li><a href="{{ request()->fullUrlWithQuery(['page' => $sonSayfa]) }}">{{$sonSayfa}}</a></li>
                                    @elseif($page+$x == $sonSayfa-1)
                                        <li><a href="{{ request()->fullUrlWithQuery(['page' => $sonSayfa]) }}">{{$sonSayfa}}</a></li>
                                    @endif

                                    @if($page < $sonSayfa)
                                        <li><a class="next" href="{{ request()->fullUrlWithQuery(['page' => $page+1]) }}">Sonraki</a></li>
                                    @endif
                                @endif
                            </ul>
                        </div>
